...
function loadFromLocalStorage() {
  const storedFavorites = localStorage.getItem('gule-favorites');
  const storedCompleted = localStorage.getItem('gule-completed');
...
function saveToLocalStorage() {
  localStorage.setItem('gule-favorites', JSON.stringify(favorites));
  localStorage.setItem('gule-completed', JSON.stringify(completed));
}
...
async function loadMaterials() {
  try {
    const response = await fetch('https://syari-api.onrender.com/gule-materials.json');
    if (!response.ok) {
